...
controls<-phydata.filt@sam_data$Sample.R_cat %in% c("ExtBlank","LibBlank")

# filter contaminants that are in blanks or are significantly associated with the amount of DNA input for sequencing
both.contaminants <- isContaminant(phydata.filt, method="either",neg = controls,conc = "Seq.copies.in.pool",threshold = c(0.1,0.5),normalize = T)

# how many contaminants were filtered out?
table(both.contaminants$contaminant) #0
...
##removing environemntal contaminants (if still present)
env.exogenous.id.list <- read.table("~/MEME/Uppsala_Katja_Project/Metagenomics/data/env-exogenous-id-list.txt", quote="\"", comment.char="", stringsAsFactors=TRUE)
envID <- env.exogenous.id.list$V1
envID <- str_replace(envID, "^", "X")
phywocont <- prune_taxa(envID, phywocont)
...
write.table(out$out, file = "./images/ANCOM/1404ecowcolour.txt")
pdf(file = "./images/ANCOM/1404ecowcolour.pdf", height = 5, width = 12)
