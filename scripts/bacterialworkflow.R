...
bac_otu <- read.delim("./data/kraken2_otu_table_merged_201129_bacteria.txt", na.strings = c("","NA"), stringsAsFactors=FALSE) %>% 
  select(which(colSums(.) > 0)) %>%   # remove empty taxa
  filter(rowSums(.) > 0) # remove empty samples

colnames(bac_otu) <- str_replace(colnames(bac_otu), "X", "")
bacOTU <- otu_table(bac_otu, taxa_are_rows = FALSE)
...
bacOTUtaxa <- taxize::classification(colnames(bac_otu), db = "ncbi")

bound1bac<-bind_rows(as_tibble(cbind(bacOTUtaxa))) %>%
  select(phylum,class,order,family,genus,species)
rownames(bound1bac)<-names(bacOTUtaxa)

write.csv(bound1bac, "./data/bacOTUtaxonomyformatted.csv")

#reading and formatting taxonomy table, made with classification
bacOTUtaxonomyformatted <- read.csv("./data/bacOTUtaxonomyformatted.csv", row.names=1, stringsAsFactors=FALSE) %>% # read in taxa table saved from taxize 
  rename_all(str_to_title)  # make the column names into title case
bactaxotable <- tax_table(as.matrix(bacOTUtaxonomyformatted))

#[sample_names(bacOTU),
sampledata <- sample_data(metadata) # only take the samples that are present in the OTU table

# making full phyloseq data format
bacphy <- phyloseq(bacOTU, sampledata, bactaxotable)
