...
length(filteredout)
blankabundance <- sample_names(phydata.filt)[which(grepl(c("BE|BL|Bk"),sample_names(phydata.filt)))]
phyabundance.blanks <- prune_samples(blanksamples,phydata.filt)
abundanceblanks <- data.frame(otu_table(phyabundance.blanks)) %>%
  select(which(colSums(.) > 0))  # remove empty taxa
abundanceblank <- colnames(abundanceblanks)
phyabundanceblanks <- prune_taxa(blank, phyabundance.blanks) #282 taxa

##filter out taxa that are just in swabs and samples (i.e. not in blanks)
swabsamples <- sample_names(phydata.filt)[which(grepl("^BS",sample_names(phydata.filt)))]
phy.swabs <- prune_samples(swabsamples, phydata.filt)
otu.swabs <- data.frame(otu_table(phy.swabs)) %>%
  select(which(colSums(.) > 0))  # remove empty taxa
swab <- colnames(phy.swabs)
physwabs <- prune_taxa(swab, phy.swabs) #348 taxa


##how many overlap between swabs and blanks?
x <- !taxa_names(physwabs) %in% taxa_names(phyabundanceblanks)
onlyswabs <- prune_taxa(x, physwabs) #282 taxa
y <- !taxa_names(phydata.filt) %in% taxa_names(onlyswabs)
phywoswabs <- prune_taxa(y, phydata.filt) #66 taxa removed
...
both.contaminants <- isContaminant(phywoswabs, method="either",neg = controls,conc = "Seq.copies.in.pool",threshold = c(0.1,0.5))
